<!-- 
  This example shows a COMPLETE persona.md with tasks.
  In practice, the coding agent creates only the Background + Environment +
  Agent Instructions sections. Then generate_tasks.py asks the persona LLM
  to generate the Requirements section. The tasks below were generated by
  the persona agent, not the coding agent.
-->
