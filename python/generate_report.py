...
def _load_prototype_results(experiment_dir: str) -> list[dict]:
    ...
    for entry in sorted(os.listdir(experiment_dir)):
        ...
        # best/ uses underscored branch slug: autoproto_<exp>_<proto>
...
def _load_best_scores_from_history(experiment_dir: str) -> dict[str, float]:
    """Load the best kept composite score per branch from iteration_history.jsonl or autoproto.jsonl.
    ...
    # Try autoproto.jsonl first (in parent directory of experiment_dir — the project root)
    # autoproto.jsonl is at project root, experiment_dir is e.g. results/recipe/
    project_root = experiment_dir
    for _ in range(3):  # Walk up max 3 levels
        jsonl_path = os.path.join(project_root, "autoproto.jsonl")
            ...
            try:
                with open(jsonl_path) as f:
                    for line in f:
                        ...
                        # Extract proto id from branch name like "autoproto/recipe/proto-a"
...
def generate_report(
    ...
    lines.append("")
    lines.append("---\n")
    lines.append("*This report was generated by autoproto. All synthetic user feedback should be treated as hypotheses to validate with real users, not as conclusions.*")
