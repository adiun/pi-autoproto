...
// Per-task timeout: 180s should cover ~15 steps with vision mode
const PER_TASK_TIMEOUT_S = 180;

// Whole-run timeout multiplier (for calibrate/variants modes that run everything at once)
const TIMEOUT_PER_TASK_S = 180;
...
export function registerEvaluateTool(pi: ExtensionAPI, getRuntime: () => AutocritRuntime) {
	...
	async function runPerTask(
		...
		const existing = readEvalResults(runCtx.evalResultsPath);
		if (existing) {
			combined.tasks = existing.tasks;
		}
	...
	async function runAllAtOnce(
		...
		// Estimate task count for timeout
		const taskNumbers = parsePersonaTaskNumbers(
			runCtx.ctx.cwd,
			params.requirements_file as string | undefined,
		);
		...
		const timeout = TIMEOUT_BASE_S + (numTasks * TIMEOUT_PER_TASK_S * runsMultiplier);

		runCtx.onUpdate?.({
			content: [{
				type: "text",
				text: `Running evaluation (iteration ${params.iteration}, mode: ${runCtx.mode}, timeout: ${Math.round(timeout / 60)}min)…`,
			}],
		});
